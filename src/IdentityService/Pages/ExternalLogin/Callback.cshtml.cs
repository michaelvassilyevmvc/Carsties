...
public class Callback : PageModel
{
        ...
        IEnumerable<Claim> claims)
    {
        ...
        // email
        var email = claims.FirstOrDefault(x => x.Type == JwtClaimTypes.Email)
                        ?.Value ??
        ...
        {
            var first = claims.FirstOrDefault(x => x.Type == JwtClaimTypes.GivenName)
                            ?.Value ??
                        claims.FirstOrDefault(x => x.Type == ClaimTypes.GivenName)
                            ?.Value;
            var last = claims.FirstOrDefault(x => x.Type == JwtClaimTypes.FamilyName)
                           ?.Value ??
        ...
        if (!identityResult.Succeeded)
            throw new InvalidOperationException(identityResult.Errors.First()
                .Description);
